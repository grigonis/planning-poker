...
| D039 | M008 | security | Input sanitization | Central `sanitize(str, maxLen)` helper in roomHandlers | Consistent string trimming + length caps across all socket event handlers | No |
| D040 | M008 | security | Vote value whitelist | Server validates cast_vote value against room.votingSystem.values | Prevents arbitrary string injection via vote events; UNIVERSAL_SPECIALS list covers cross-system special cards | No |
| D041 | M008 | arch | SocketContext reconnect dependency | Use authUid (string) not authUser (object) as effect dep | Firebase recreates User object on token refresh; using object reference as dep caused spurious socket teardown/reconnect cycles which broke in-flight emit callbacks | No |
| D042 | M008 | arch | AudioContext singleton | Single AudioContext created at Room mount, reused per vote event | Browser limits concurrent AudioContexts (6-32); creating one per event silently fails after limit | No |
| D043 | M008 | ops | Room TTL cleanup | setInterval sweeps abandoned rooms every 15min; TTL=4h | Prevents server memory exhaustion from rooms where host closed tab without ending session | Yes — TTL adjustable |
